...
const BASE_URL =
  "https://s40lp1.ucc.cit.tum.de/sap/opu/odata/sap/ZSB_PO_HEADER_203_2";
const CLIENT = "324";
...
export async function fetchPOHeaders(
  ...
  if (filters.comp_code)
    filterClauses.push(`comp_code eq '${filters.comp_code}'`);
  if (filters.vendor) filterClauses.push(`vendor eq '${filters.vendor}'`);
  if (filters.purch_org)
    filterClauses.push(`purch_org eq '${filters.purch_org}'`);
